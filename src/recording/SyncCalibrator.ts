...
export class SyncCalibrator extends EventEmitter {
        ...
        try {
            if (fsSync.existsSync(this.historyFile)) {
                const data = JSON.parse(
                    fsSync.readFileSync(this.historyFile, 'utf8'),
                )
                console.log(
                    `📊 Loaded sync learning data: ${data.history.length} sessions`,
                )
                return data
            }
        } catch (error) {
    ...
    private saveLearningData(): void {
        try {
            ...
            console.log(
                `💾 Sync learning data saved: ${this.learningData.history.length} sessions`,
            )
        } catch (error) {
    ...
    private updateStatistics(): void {
        ...
        // Calculer moyennes
        const avgDetected =
            history.reduce((sum, entry) => sum + entry.detectedOffset, 0) /
            history.length
        const avgEmpirical =
            history.reduce((sum, entry) => sum + entry.empiricalAdjustment, 0) /
            history.length
    ...
    public getIntelligentEmpiricalAdjustment(detectedOffset: number): number {
        ...
        if (stats.totalSessions < 3) {
            ...
            console.log(
                `🎯 LEARNING MODE: Using default 100ms (only ${stats.totalSessions} sessions)`,
            )
            return 0.1
        }
        ...
        if (stats.confidence > 0.8) {
            ...
            console.log(
                `🧠 HIGH CONFIDENCE: Using learned adjustment ${stats.averageEmpiricalAdjustment.toFixed(3)}s (confidence: ${stats.confidence.toFixed(2)})`,
            )
            ...
            const smartAdjustment =
                (1 - weight) * 0.1 + weight * stats.averageEmpiricalAdjustment
            console.log(
                `🤖 SMART BLEND: ${smartAdjustment.toFixed(3)}s (${(weight * 100).toFixed(0)}% learned, ${((1 - weight) * 100).toFixed(0)}% default)`,
            )
            return smartAdjustment
        }
    }

    public recordSyncSession(
        detectedOffset: number,
        empiricalAdjustment: number,
    ): void {
        ...
        console.log(
            `📝 Recorded sync session: detected=${detectedOffset.toFixed(3)}s, empirical=${empiricalAdjustment.toFixed(3)}s`,
        )
    }
    ...
    public provideFeedback(
        ...
        if (
            sessionIndex >= 0 &&
            sessionIndex < this.learningData.history.length
        ) {
            this.learningData.history[sessionIndex].userFeedback = feedback
            this.saveLearningData()
            console.log(
                `💭 User feedback recorded: ${feedback} for session ${sessionIndex}`,
            )
    ...
    private adjustBasedOnFeedback(): void {
        ...
        const goodSessions = feedbackSessions.filter(
            (s) => s.userFeedback === 'good' || s.userFeedback === 'perfect',
        )
        const badSessions = feedbackSessions.filter(
            (s) => s.userFeedback === 'bad',
        )
        ...
        if (goodSessions.length > badSessions.length) {
            ...
            console.log(
                '⚠️ Negative feedback trend - adjusting learning parameters...',
            )
    ...
    public showLearningProgress(): void {
        ...
        console.log(
            `📈 Average detected offset: ${stats.averageDetectedOffset.toFixed(3)}s`,
        )
        console.log(
            `🎯 Learned empirical adjustment: ${stats.averageEmpiricalAdjustment.toFixed(3)}s`,
        )
        console.log(`📊 Variance (stability): ${stats.variance.toFixed(3)}`)
        console.log(
            `🎯 System confidence: ${(stats.confidence * 100).toFixed(1)}%`,
        )
        ...
        if (stats.totalSessions >= 10) {
            ...
            console.log(
                '🔴 System needs more sessions to learn optimal settings',
            )
        }
    }
    ...
    public async generateSyncSignal(page: any): Promise<void> {
        ...
        try {
            ...
            console.error(
                'Failed to generate ultra-precise sync signal:',
                error,
            )
            throw error
        }
     ...
     */
    public async applySyncCorrection(
        inputPath: string,
        outputPath: string,
        audioOffset: number,
    ): Promise<void> {
        ...
        return new Promise((resolve, reject) => {
            const args = [
                ...
                inputPath,
                '-itsoffset',
                audioOffset.toString(),
                '-i',
                inputPath,
                '-map',
                '0:v:0', // Vidéo du premier input
                '-map',
                '1:a:0', // Audio du deuxième input (avec offset)
                '-c:v',
                'copy', // Copier la vidéo sans réencodage
                '-c:a',
                'aac', // Réencoder l'audio
                '-y',
                outputPath,
            ]
    ...
    private async detectGreenFlash(videoPath: string): Promise<number> {
        ...
        try {
            ...
            const extractCmd = [
                ...
                videoPath,
                '-vf',
                'fps=20', // 20fps = 0.05s intervals pour +PRÉCISION
                '-y',
                path.join(framesDir, 'frame_%03d.png'),
            ]

            console.log(
                '🎬 Extracting frames for ULTRA-PRECISE green flash detection...',
            )
            ...
            console.log(
                `🔍 Analyzing ${pngFrames.length} frames at 20fps for precision...`,
            )
            ...
            for (let i = 0; i < pngFrames.length; i++) {
                ...
                console.log(
                    `Frame ${i + 1}: time=${frameTime.toFixed(2)}s, green=${greenIntensity.toFixed(1)}`,
                )
            }
            ...
            if (maxGreenIntensity > 100) {
                console.log(
                    `✅ ULTRA-PRECISE Green flash detected! Duration: ${flashStart}s → ${flashEnd}s, peak: ${maxGreenTime}s`,
                )
                return flashStart // Utiliser le début du flash
            } else {
                console.log(
                    `❌ No green flash found (max intensity: ${maxGreenIntensity.toFixed(1)})`,
                )
                return -1
            }
        } catch (error) {
    ...
    private async detectBeep(videoPath: string): Promise<number> {
        console.log(
            '🔊 Starting REAL audio beep detection (no more fake estimation!)...',
        )
        ...
        try {
            ...
            console.log(
                '🔍 ULTRA-PRECISE audio spectrum analysis for 1000Hz beep...',
            )
            ...
            console.log(
                `🎵 Analyzing ${lines.length} audio samples for beep detection...`,
            )
            ...
            for (const line of lines) {
                ...
                if (parts.length >= 2) {
                    ...
                    if (!isNaN(timestamp) && !isNaN(peakLevel)) {
                        console.log(
                            `🎵 Audio analysis: t=${timestamp.toFixed(3)}s, peak=${peakLevel.toFixed(1)}dB`,
                        )

                        // SEUIL AJUSTÉ : -12dB au lieu de -15dB pour détecter les pics observés
                        if (peakLevel > maxPeak && peakLevel > -12) {
                            // PRÉCISION: Seuil à -12dB
                            maxPeak = peakLevel
                            beepTime = timestamp
                            console.log(
                                `🎯 NEW PEAK DETECTED: ${peakLevel.toFixed(1)}dB at ${timestamp.toFixed(3)}s`,
                            )
                        }
                    }
                }
            ...
            if (beepTime > 0 && beepTime < 5) {
                console.log(
                    `✅ ULTRA-PRECISE beep detected at ${beepTime.toFixed(3)}s (peak: ${maxPeak.toFixed(1)}dB)`,
                )
                return beepTime
            } else {
                console.log(
                    `❌ No beep detected (max peak: ${maxPeak.toFixed(1)}dB at ${beepTime.toFixed(3)}s)`,
                )
                return -1
            }
        } catch (error) {
            ...
            console.log(
                '🔄 SMART FALLBACK: Using temporal correlation with flash detection...',
            )
    ...
    public async calibrateOnce(page: any): Promise<number> {
        ...
        try {
            ...
            console.log(
                '📹 Recording 3-second ULTRA-PRECISE calibration video...',
            )

            // Capture x11grab AVEC framerate élevé pour plus de précision temporelle
            const capturePromise = execAsync(
                `ffmpeg -f x11grab -video_size 1280x880 -framerate 30 -t 3 -i :99 -f pulse -t 3 -i virtual_speaker.monitor -c:v libx264 -c:a aac -map 0:v:0 -map 1:a:0 -y ${calibrationPath}`,
            )
            ...
            setTimeout(async () => {
                try {
                    console.log(
                        '🎯 Generating ULTRA-PRECISE calibration sync signal...',
                    )
                    await this.generateSyncSignal(page)
                    console.log(
                        '🎯 ULTRA-PRECISE calibration sync signal generated',
                    )
                } catch (error) {
                    console.warn(
                        '⚠️ Failed to generate calibration signal:',
                        error,
                    )
                }
            ...
            console.log(
                '⏳ Waiting for ULTRA-PRECISE calibration video capture to complete...',
            )
            ...
            console.log(
                '🔍 Starting ULTRA-PRECISE sync analysis of calibration video...',
            )
            ...
            try {
                ...
                console.warn(
                    '⚠️ Failed to cleanup calibration file:',
                    cleanupError,
                )
            }
            ...
            if (syncResult.confidence > 0.5) {
                console.log(
                    `🎯 ULTRA-PRECISE CALIBRATION SUCCESS: ${syncResult.audioOffset.toFixed(3)}s offset detected`,
                )
                console.log(
                    `📊 Confidence: ${syncResult.confidence}, Flash: ${syncResult.flashTimestamp.toFixed(3)}s, Beep: ${syncResult.beepTimestamp.toFixed(3)}s`,
                )
                return syncResult.audioOffset
            } else {
                console.warn(
                    '⚠️ Low confidence calibration, using smart fallback offset',
                )
                console.log(
                    `📊 Low confidence details: ${syncResult.confidence}, Flash: ${syncResult.flashTimestamp.toFixed(3)}s, Beep: ${syncResult.beepTimestamp.toFixed(3)}s`,
                )
                ...
                if (syncResult.flashTimestamp > 0) {
                    ...
                    const estimatedOffset =
                        estimatedBeepTime - syncResult.flashTimestamp // = 0 (synchrone)
                    console.log(
                        `🎯 SMART FALLBACK: Using flash-based sync estimate (beep=${estimatedBeepTime.toFixed(3)}s, offset=${estimatedOffset}s)`,
                    )
    ...
    public async quickCalibrateOnce(page: any): Promise<number> {
        ...
        try {
            ...
            const capturePromise = execAsync(
                `ffmpeg -f x11grab -video_size 1280x880 -framerate 30 -t 2 -i :99 -f pulse -t 2 -i virtual_speaker.monitor -c:v libx264 -c:a aac -map 0:v:0 -map 1:a:0 -y ${calibrationPath}`,
            )
            ...
            if (syncResult.confidence > 0.5) {
                console.log(
                    `Quick calibration success: ${syncResult.audioOffset.toFixed(3)}s (confidence: ${syncResult.confidence.toFixed(2)})`,
                )
                return syncResult.audioOffset
            } else {
    ...
    public async quickCalibrateOnceOptimized(page: any): Promise<number> {
        ...
        try {
            ...
            const capturePromise = execAsync(
                `ffmpeg -f x11grab -video_size 640x480 -framerate 15 -t 1.5 -i :99 -f pulse -t 1.5 -i virtual_speaker.monitor -c:v libx264 -preset ultrafast -crf 30 -c:a aac -b:a 64k -map 0:v:0 -map 1:a:0 -y ${calibrationPath}`,
            )
            ...
            if (syncResult.confidence > 0.5) {
                console.log(
                    `Optimized calibration success: ${syncResult.audioOffset.toFixed(3)}s (confidence: ${syncResult.confidence.toFixed(2)})`,
                )
                return syncResult.audioOffset
            } else {
