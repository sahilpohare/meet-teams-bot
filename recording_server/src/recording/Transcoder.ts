...
export class Transcoder extends EventEmitter {
    ...
    public configure(
        ...
        if (!transcriptionService) {
            throw new Error(
                'TranscriptionService is required for configuration',
            )
        }
    ...
    private validateStartConditions(): void {
        if (
            ...
            throw new Error(
                'Transcoder must be configured with PathManager and TranscriptionService before starting',
            )
        }
        ...
        if (this.isAudioOnly) {
            if (!this.config.outputPath.endsWith('.mp3')) {
                throw new Error(
                    'Output path must have .mp3 extension in audio-only mode',
                )
            }
        } else {
    ...
    public async start(): Promise<void> {
        ...
        try {
            ...
            await Promise.all([this.startFFmpeg(), this.startWebmStream()])
    ...
    private async processTranscriptionSegment(
        ...
        this.videoProcessor.on('chunkProcessed', async ({ metadata }) => {
            ...
            if (segment) {
                try {
                    ...
                    console.error(
                        'Error handling transcription segment:',
                        error,
                    )
                    this.emit('error', { type: 'transcriptionError', error })
                }
    ...
    private async optimizeVideo(): Promise<void> {
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            fastStartProcess.on('close', async (code) => {
                ...
                if (code === 0) {
                    try {
                        await this.pathManager!.moveFile(
                            tempPath,
                            this.config.outputPath,
                        )
                        resolve()
    ...
    public async uploadToS3(): Promise<string> {
        ...
        console.log(
            `Uploading ${this.isAudioOnly ? 'audio' : 'video'} to S3:`,
            {
                localPath,
        ...
        return this.s3Uploader.uploadFile(
            localPath,
