    ...
    private async requestTranscription(
        ...
        try {
            ...
            const transcribeResponse =
                response.data as TranscribeRequestResponse
        ...
        while (true) {
            ...
            try {
                ...
                if (response.status !== this.DONE_HTML_CODE) {
                    throw new Error(
                        `Result retrieval failed with status ${response.status}`,
                    )
                }
                ...
                if (result.status === 'error') {
                    ...
                    throw new Error(
                        'Transcription failed: ' + JSON.stringify(result),
                    )
                }
