...
import { TranscriptionProvider, TranscriptionResult } from './providers/TranscriptionProvider';
...
interface TranscriptionServiceOptions extends TranscriptionOptions {
    onTranscriptionComplete?: (results: TranscriptionResult[], segment: TranscriptionSegment) => Promise<void>;
}
...
export class TranscriptionService extends EventEmitter {
    ...
    private setupQueue() {
        this.transcriptionQueue = queue<TranscriptionTask>(async (task, callback) => {
            if (this.isPaused) {
                callback();
                return;
            }
    ...
    private async processSegment(segmentId: string): Promise<void> {
        ...
        try {
            ...
            console.error(`Transcription error for segment ${segmentId}:`, error);
    ...
    public async resume(): Promise<void> {
        ...
        const failedSegments = Array.from(this.segments.values())
            .filter(s => s.status === 'failed' && s.retryCount < (this.options.maxRetries || 3));
    ...
    public async stop(): Promise<void> {
        try {
            this.isPaused = true;
            await new Promise<void>(resolve => this.transcriptionQueue.drain(resolve));
            this.emit('stopped');
        } catch (error) {
    ...
    public getSegmentStatus(segmentId: string): TranscriptionSegment | undefined {
