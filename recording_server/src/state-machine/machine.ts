import { MeetingStateType, StateTransition } from "./types";
...
export class MeetingStateMachine {
    ...
    public async start(): Promise<void> {
        try {
            while (this.currentState !== MeetingStateType.Cleanup && !this.forceStop) {
                ...
                if (this.forceStop) {
                    this.context.endReason = this.context.endReason || 'forced_stop';
    ...
    private async handleError(error: Error): Promise<void> {
        try {
            ...
            const errorState = getStateInstance(MeetingStateType.Error, this.context);
    ...
    private async transitionToCleanup(): Promise<void> {
        ...
        const cleanupState = getStateInstance(MeetingStateType.Cleanup, this.context);
