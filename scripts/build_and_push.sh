...
DEFAULT_BUILD_CONTEXT="apps/bot"  # Monorepo: bot is in apps/bot/
...
PROJECT DEFAULTS:
    ...
    Dockerfile:    ${DEFAULT_DOCKERFILE}
    Platform:      ${DEFAULT_PLATFORM}
    Build Context: ${DEFAULT_BUILD_CONTEXT}
...
# Verify package.json exists in build context
if [ ! -f "${BUILD_CONTEXT}/package.json" ]; then
    print_error "package.json not found in ${BUILD_CONTEXT}"
    exit 1
fi
...
BUILD_CMD="$BUILD_CMD --platform ${PLATFORM} -f ${BUILD_CONTEXT}/${DOCKERFILE} -t ${FULL_IMAGE_NAME} ${BUILD_CONTEXT}"
